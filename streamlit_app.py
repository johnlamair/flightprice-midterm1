...
if selected_page == "Introduction 📘":
    ...
    st.write(
        "Develop a predictive model that estimates flight prices "
        "based on variables that affect ticket pricing."
    )

    st.markdown("### Dataset")
    st.write(
        "Flight booking data from Easemytrip for India's top 6 metro cities. "
        "300,261 datapoints and 11 features."
    )
    ...
    for tab, airline_name in zip(tabs, ["Overall"] + airline_names):
        with tab:
            if airline_name == "Overall":
                df_filtered = flight_data[["SourceCity", "DestinationCity", "Price", "DepartureTime", "Airline"]].copy()
            else:
                df_filtered = flight_data[flight_data["Airline"] == airline_name][["SourceCity", "DestinationCity", "Price", "DepartureTime"]].copy()
            ...
            # heatmap of prices
            heatmap_matrix = df_filtered.pivot_table(
                index='SourceCity',
            ...
            avg_price_by_departure = df_filtered.groupby('DepartureTime')['Price'].mean().reindex(departure_time_order)
    ...
    features = ["Airline", "FlightNumber", "SourceCity", "DepartureTime",
                "Stops", "ArrivalTime", "DestinationCity", "Class",
                "Duration", "DaysLeft"]
    target = "Price"

    # encode categorical features
    categorical_features = ["Airline", "FlightNumber", "SourceCity", "DepartureTime",
                            "Stops", "ArrivalTime", "DestinationCity", "Class"]
    for feature in categorical_features:
        flight_model_data[feature] = flight_model_data[feature].astype("category").cat.codes

    # create tabs for each airline
